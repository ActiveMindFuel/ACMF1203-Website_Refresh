...
class cactus_masonry_plus {	
	...
	private static $VERSION = "0.0.5.0";
	...
	static public function cactus_masonry_shortcode_handler($atts) {
		...
		if(self::$a['paginate']) {
			...
			if(self::$a['offset'] == null) {
				if(self::$a['paginate'] && isset($_GET["paged"])) {
					$p = intval($_GET["paged"]);
					if($p != 0) {
						$page = $p;
		...
		if($the_query->have_posts()) {
			...
			while($the_query->have_posts()) {
				...
				if(self::$a["gallerymode"] && get_post_gallery()) {
					...
					//Add meta data
					if(self::$a['showtitle']) $o .= "b.title = \"" . htmlspecialchars(get_the_title()) . "\";";
					...
					if(self::$a['showauthor']) {
						...
						$o .= "b.author = \"" . htmlspecialchars($auth) . "\";";
						if(self::$a['showauthoraslink']) $o .= "b.authorUrl = \"" . get_author_posts_url(get_the_author_meta("ID"), $auth) . "\";";
					}
					if(self::$a['showdate']) {
						if(self::$a['dateformat'] == "") $o .= "b.date = \"" . htmlspecialchars(get_the_date()) . "\";";
						else $o .= "b.date = \"" . htmlspecialchars(get_the_date(self::$a['dateformat'])) . "\";";
					}
					//Load or lazy load gallery brick
					if(self::$a['lazyload']) $o .= "cmp{$id}.queueBrick(b);";
					else $o .= "cmp{$id}.addBrick(b);";
